...
for th_id in threads_list:
    ...
    for i in range(1, 23):
        ...
        print(df)
        plan_timings = None
        plan_input_left = plan_input_right = plan_output = None
        if args.profile:
            plan_timings = parse_plan_timings(args.qid)

            plan_input_left, plan_input_right, plan_output = parse_plan_cardinality(args.qid)
        ...
        physical_op_size = None
        if args.lineage and args.stats:
            lineage_size, lineage_count, nchunks, postprocess_time, plan, physical_op_size = getStats(con, query)
            print(plan)
        ...
        #     oid = int(re.findall(r'\d+', key)[0])
        #     print(key, plan_output[key], plan_input_left[key], plan_input_right[key], plan_timings[key], physical_op_size[oid])
